...
function toggleDiagPort(successOnCb, successOffCb, errorCb) {
	...
	if (status === activeState) {
		//diag enabled, disabling
		setSystemProperty(sysProp, inactiveState);
		successOffCb();
	} else {
		//diag disabled, enabling
...
function setSystemProxyConfig(config) {
	var allowedPrefs = [
		...
		pref;
	for (pref in config) {
		if (allowedPrefs.indexOf(pref) > -1) setSystemPreference("network.proxy." + pref, config[pref]);
	}
}
...
function calcIMEIChecksum(imei) {
	if (imei === "" + imei)
		//split the digits if we passed the string
...
function setMtkWlanMAC(mac, successCb, errorCb) {
	var targetFile = "/data/nvram/APCFG/APRDEB/WIFI",
		tmpFile = "/cache/mac",
		startOffset = 4,
		dPayload =
			"\\x" +
...
function setMtkBluetoothMAC(mac, successCb, errorCb) {
	var targetFile = "/data/nvram/APCFG/APRDEB/BT_Addr",
		tmpFile = "/cache/mac",
		dPayload =
			"\\x" +
			mac
...
function enableCallRecording(flag = "on", format = "wav", successCb, errorCb) {
	setSystemSetting(
		...
		function () {
			setSystemSetting(
				"callrecording.file.format",
				format,
				function () {
					setSystemSetting(
						"callrecording.notification.enabled",
						false,
						function () {
